...
import { MoviesService } from '../../../layout/components/Movie/services/movies.service';
import { Component, Input, OnInit } from '@angular/core';
import { OwlOptions } from 'ngx-owl-carousel-o';
import { HomeService } from 'src/app/layout/services/home.service';
...
export class SliderComponent implements OnInit {
  ...
  getMovieTrailer(movie_id: number) {
    this._SharedService.getTrailers('movie',movie_id).subscribe((response) => {
  ...
  ShowMovie(id: number) {
    this._SharedService.getDetails('movie',id.toString()).subscribe(
      (response) => {
        this.Movie = [response];
      },
      (error) => {
  ...
  ngOnInit(): void {
    this._SharedService.getTrending('movie',1).subscribe((response) => {
